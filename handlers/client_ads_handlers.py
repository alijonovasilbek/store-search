...
@ads_router.message(Command('search'))
async def search_handler(message: Message, state: FSMContext):
    await state.set_state(ClientAdsStates.searchAds)
    await message.answer("Iltimos so'rovni kiriting: ")


@ads_router.message(ClientAdsStates.searchAds)
async def search_query_handler(message: Message, state: FSMContext):
        search_query = message.text


        search_results = db.search_ads(search_query)

        if search_results:
            search_results_texts = [f"{i+1}. {result}" for i, result in enumerate(search_results)]

            await state.set_state(ClientAdsStates.showSearchResults)
            await state.update_data(search_results=search_results_texts)

            buttons_per_row = 5
            buttons = [
                InlineKeyboardButton(text=str(i+1), callback_data=f"select_result_{i}")
                for i in range(len(search_results_texts))
            ]

            buttons_row1 = buttons[:buttons_per_row]
            buttons_row2 = buttons[buttons_per_row:]

            inline_keyboard = InlineKeyboardMarkup(inline_keyboard=[buttons_row1, buttons_row2])

            search_results_with_titles = "\n".join([f"{i+1}. {result[0]}\n" for i,result in  enumerate(search_results)])



            await message.answer(f"Search results: \n\n {search_results_with_titles}", reply_markup=inline_keyboard)
        else:
         await message.answer("No ads found for your search query.")

@ads_router.callback_query(ClientAdsStates.showSearchResults)
async def show_search_results_handler(callback: CallbackQuery, state: FSMContext):
    all_data = await state.get_data()
    search_results_texts = all_data.get('search_results')

    if callback.data.startswith('select_result_'):
        selected_index_str = callback.data.split('_')[-1]
        try:
            selected_index = int(selected_index_str)
            if 0 <= selected_index < len(search_results_texts):
                selected_result_text = search_results_texts[selected_index]
                a = eval(str(selected_result_text).split(".",1)[1].strip())
                selected_result_text1 = f"Turkumi: {a[0]} 📱\n\nMalumot : {a[1]}\n\nNarxi: {a[2]} $"

                await callback.message.answer(selected_result_text1)


                buttons = []


                total_pages = (len(search_results_texts) + 4) // 5

                current_page = selected_index // 5 + 1

                if current_page > 1:
                    buttons.append(InlineKeyboardButton(text="Oldingi", callback_data=f"page_{current_page - 1}"))

                if current_page < total_pages:
                    buttons.append(InlineKeyboardButton(text="Keyingi", callback_data=f"page_{current_page + 1}"))
                if len(search_results_texts) > 5:
                    inline_keyboard = InlineKeyboardMarkup(inline_keyboard=[buttons])
                    await callback.message.answer("Sahifalash", reply_markup=inline_keyboard)
