...
class Database:
    ...
    def search_ads(self, search_query):
        self.cursor.execute("SELECT ad_title, ad_text, ad_price FROM ads WHERE ad_title LIKE ?", ('%' + search_query + '%',))
        search_results = self.cursor.fetchall()
        return search_results if search_results else None

    def all_images(self,infor_id):
        self.cursor.execute("SELECT ad_images FROM ads WHERE id=?;",(infor_id))
